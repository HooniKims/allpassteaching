...
- StandardsLibrary keeps AI recommendations and the official curriculum list as separate, simultaneously available sections for every subject. Both feed one `현재 / 최대 10개` selection summary; middle-school social studies applies the additional geography/general-social-studies/history scope, and mobile stacks search and area controls without hiding the official list.
...
- AssessmentDesignFlow leads with the confirmed lesson title and achievement standards, then one AI draft action. AI-populated options live in a native `details` disclosure; the teacher edits the task and rubric before a separate student-sheet action, and changing that design visibly invalidates the older student document without deleting it.
