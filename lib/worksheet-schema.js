...
/**
 * 학습지 렌더러가 문항 번호를 따로 붙이므로, 모델이 지문 앞에 넣은 번호는 지웁니다.
 * 앞머리 숫자가 그 문항의 전체 순번이나 섹션 안 순번과 같을 때만 지워 '2022년…' 같은 지문은 건드리지 않습니다.
 */
export function withoutQuestionNumbering(worksheet) {
    let number = 0;
    return {
        ...worksheet,
        document: {
            ...worksheet.document,
            sections: worksheet.document.sections.map(section => {
                let sectionNumber = 0;
                return {
                    ...section,
                    questions: section.questions.map(question => {
                        number += 1;
                        sectionNumber += 1;
                        const prompt = question.prompt.replace(new RegExp(`^\\s*(?:${number}|${sectionNumber})\\s*[.)]\\s+`), '');
