...
const normalizedEvidenceText = value => String(value ?? '').normalize('NFKC').replace(/\s+/g, '');

export function gradingEvidenceMatchesElements(evidence, elements = []) {
    // OCR wraps lines mid-sentence, so compare on whitespace-stripped text — evidence quoted
    // without the original line breaks must still count as the same passage.
    const expected = normalizedEvidenceText(evidence);
    return expected.length > 0 && elements.length > 0 && elements.every(element => {
        const actual = normalizedEvidenceText(element?.text);
        return actual.length > 0 && (actual.includes(expected) || expected.includes(actual));
    });
}

export function sharedGradingEvidenceElementIds(criteria = []) {
    const ownersByElementId = new Map();
    for (const criterion of criteria) {
        for (const sourceRef of criterion.sourceRefs ?? []) {
            const owners = ownersByElementId.get(sourceRef.elementId) ?? new Set();
            owners.add(criterion.criterionId);
            ownersByElementId.set(sourceRef.elementId, owners);
        }
    }
    return new Map([...ownersByElementId].filter(([, owners]) => owners.size > 1).map(([elementId, owners]) => [elementId, [...owners]]));
}
...
export function canonicalGradingProvenance(value = {}) {
    return {
        ...
        gradingRevision: Number.isInteger(value.gradingRevision) && value.gradingRevision >= 0 ? value.gradingRevision : 0,
...
export function gradingEvidenceLineage(elements = [], criteria = [], provenance = {}) {
    ...
    const { gradingRevision: _gradingRevision, ...lineageProvenance } = canonicalGradingProvenance(provenance);
    const decisions = criteria.map(criterion => ({
        ...
        revisionEvidence: criterion.revisionEvidence ?? null,
    }));
    return { elements: normalizedElements, decisions, provenance: lineageProvenance };
}
