...
export const REQUIRED_COVER_SECTION_TYPES = Object.freeze(['subject', 'transfer-goal', 'standards', 'grasps', 'submission', 'checkpoints', 'rubric']);
...
function validateAssessmentRelationships(assessment, context) {
    ...
    if (assessment.includeStudentCover) {
        REQUIRED_COVER_SECTION_TYPES.forEach(type => {
            const matching = assessment.cover.sections.filter(section => section.type === type && section.visible);
            if (matching.length !== 1) issue(context, ['cover', 'sections'], `학생 표지에는 ${type} 연결 항목이 정확히 하나 필요합니다.`);
        });
    }
}
