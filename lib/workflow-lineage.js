...
export function gradingSourceHash(assessment, extractedText, elements = [], criteria = [], provenance = {}) {
    // Exclude client-only / volatile assessment metadata from the hash. The client stores the
    // assessment with designHash/sourceHash/approved, but the server re-parses it through
    // approvedAssessmentSchema (which drops designHash), so hashing the raw object made the
    // client and server disagree and blocked every grading approval. These fields are not
    // graded content, so both sides now hash the same canonical assessment.
    const { designHash, sourceHash: _assessmentSourceHash, approved, requiresAssessmentRegeneration, ...canonicalAssessment } = assessment ?? {};
    return sourceHash({ assessment: canonicalAssessment, extractedText, ...gradingEvidenceLineage(elements, criteria, provenance) });
}
...
export function gradingContentIsValid(assessment, grading, extractedText = '', elements = [], provenance = {}) {
    ...
    for (let index = 0; index < expected.length; index += 1) {
        ...
        if (result.revisionEvidence) {
            const revision = result.revisionEvidence;
            const beforeElement = elementById.get(revision.beforeSourceRef.elementId);
            const afterElement = elementById.get(revision.afterSourceRef.elementId);
            if (criterion.kind !== 'process'
                ...
                || !gradingSourceRefMatchesElement(revision.beforeSourceRef, beforeElement)
                || !gradingSourceRefMatchesElement(revision.afterSourceRef, afterElement)
                || !gradingEvidenceMatchesElements(revision.beforeEvidence, [beforeElement])
                || !gradingEvidenceMatchesElements(revision.afterEvidence, [afterElement])) return false;
        }
        ...
        // Mirror the server finalize rule: one OCR element can back only one scored criterion.
        for (const ref of result.sourceRefs) {
            const owner = evidenceOwnerByElementId.get(ref.elementId);
            if (owner && owner !== result.criterionId) return false;
            evidenceOwnerByElementId.set(ref.elementId, result.criterionId);
        }
...
export function recordSourceHash(assessment, submission) {
    const approvalToken = submission?.grading?.approvalToken ?? '';
    return /^[a-f0-9]{64}$/.test(approvalToken) ? `record-v2:${approvalToken}` : '';
}
