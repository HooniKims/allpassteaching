...
const PHASE_LABELS = Object.freeze({
    ...
    uploading: '업로드',
    parsing: 'OCR·구조 분석',
    upstageWaiting: 'Upstage 응답 대기',
    generating: 'AI 분석·생성',
    validating: '스키마 검증·보정',
    applying: '저장·화면 반영',
});

const OPAQUE_PHASES = new Set(['uploading', 'upstageWaiting']);
...
function resolveProgress(operation, phase, measuredProgress) {
    ...
    const measured = normalizeProgress(measuredProgress);
...
export function advanceOperation(operation, update) {
    ...
    const next = {
        ...
        completedResults: Object.hasOwn(update, 'completedResults')
            ? [...update.completedResults]
            : operation.completedResults,
...
export function cancelOperation(operation, cancelledAt) {
    ...
    return { ...operation, status: 'cancelled', cancelledAt: normalizeCount(cancelledAt), currentItem: null };
}
...
function formatRemainingRange(rangeSeconds) {
    ...
    if (upperMinutes === 1 && rangeSeconds[1] < 60) {
        return `약 ${Math.max(5, Math.round(rangeSeconds[0] / 5) * 5)}~${Math.max(5, Math.round(rangeSeconds[1] / 5) * 5)}초 남음`;
    }
