...
export function getOperationDisplayProgress(operation, now) {
    if (operation.progress !== null) {
        return { progress: operation.progress, progressKind: operation.progressKind, progressLabel: operation.progressLabel };
    }
    if (operation.status === 'completed') return { progress: 100, progressKind: 'actual', progressLabel: '실제 진행률' };
    const elapsedSeconds = Math.max(0, (normalizeCount(now) - operation.startedAt) / 1_000);
    const estimateSeconds = Math.max(1, operation.estimate.averageSeconds);
    const progress = Math.min(95, Math.max(1, Math.round(elapsedSeconds / estimateSeconds * 90)));
    return { progress, progressKind: 'estimated', progressLabel: '예상 진행률' };
}
