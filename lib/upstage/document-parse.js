...
function safeCoordinates(value) {
    if (!Array.isArray(value) || value.length > MAX_COORDINATE_POINTS) return [];
    const valid = value.every(point => point && Number.isFinite(point.x) && Number.isFinite(point.y)
        && point.x >= 0 && point.x <= 1 && point.y >= 0 && point.y <= 1);
    return valid ? value.map(point => ({ x: point.x, y: point.y })) : [];
}
...
export function normalizeDocumentElements(payload) {
    ...
    return source.flatMap((element, index) => {
        ...
        const confidence = Number.isFinite(element.confidence) && element.confidence >= 0 && element.confidence <= 1
            ...
            : undefined;
        const normalized = {
            id: safeElementId(element.id, index),
...
function normalizationReasons(payload, elements) {
    ...
    if (source.slice(0, MAX_ELEMENTS).some(element => !Number.isInteger(element?.page) || element.page < 1 || element.page > MAX_PAGE)) reasons.add('invalid_page');
    if (source.slice(0, MAX_ELEMENTS).some(element => Array.isArray(element?.coordinates) && safeCoordinates(element.coordinates).length === 0)) reasons.add('invalid_coordinates');
...
function normalizeResult(payload, { model, mode, visualAnalysisStatus, extraReasons = [] }) {
    ...
    const elements = normalizeDocumentElements(payload);
    const reviewReasons = [...new Set([...normalizationReasons(payload, elements), ...extraReasons])];
    ...
    return {
        ...
        ocrModel: model,
        ocrMode: mode,
        pageCount: safePageCount(payload, elements),
