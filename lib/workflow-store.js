...
import { DEFAULT_RECORD_TARGET_BYTES, normalizeRecordTargetBytes } from './record-length.js';
import { upgradeAssessmentStudentSheet } from './assessment-student-sheet.js';
import { extractAssessmentDesign } from './assessment-design.js';
...
const readableVersions = new Set([0, 1, 2, 3, 4, WORKFLOW_VERSION]);

export function createEmptyWorkflow() {
    return { activeProcess: 'lesson', lessonSnapshot: null, worksheet: null, assessmentRequest: createDefaultAssessmentRequest(), assessmentDesign: null, assessment: null, students: [], submissions: [], records: [], recordTargetBytes: DEFAULT_RECORD_TARGET_BYTES };
}

function hydrateGraspsTask(assessment) {
    if (!assessment?.task || !assessment?.backwardDesign) return assessment;
    const teacherIntent = assessment.backwardDesign.teacherIntent ?? {};
    return {
        ...assessment,
        task: {
            ...assessment.task,
            goal: assessment.task.goal?.trim() || assessment.backwardDesign.transferGoal || teacherIntent.desiredResult || '평가 목표를 확인하세요.',
            successCriteria: assessment.task.successCriteria?.trim() || teacherIntent.evidenceOfSuccess || '성취기준과 루브릭에 따른 성공 기준을 확인하세요.',
        },
    };
}
...
export function sanitizeWorkflow(value = {}) {
    ...
    const assessment = value.assessment == null ? null : upgradeAssessmentStudentSheet(hydrateGraspsTask(sanitizeAssessment(value.assessment) ?? null));
    const assessmentDesign = value.assessmentDesign == null
        ? (assessment ? sanitizeAssessmentDesign({ ...extractAssessmentDesign(assessment), sourceHash: assessment.sourceHash }) ?? null : null)
        : sanitizeAssessmentDesign(value.assessmentDesign) ?? null;
    return {
        activeProcess,
        ...
        recordTargetBytes: normalizeRecordTargetBytes(value.recordTargetBytes),
    };
}
