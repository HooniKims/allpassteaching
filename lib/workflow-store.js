...
const forbiddenStorageKeys = new Set([
    'file', 'packetFile', 'answerFile', 'objectUrl', 'objectURL',
    'rawBase64', 'rawUpstage', 'answerPdfBase64', 'pdfBase64',
    'canvas', 'canvasState', 'abortController',
]);
...
function jsonValue(value, fallback = null) {
    try {
        ...
        const serialized = JSON.stringify(value, (key, current) => {
            if (forbiddenStorageKeys.has(key)) return undefined;
            if (typeof Blob !== 'undefined' && current instanceof Blob) return undefined;
            return current;
        });
