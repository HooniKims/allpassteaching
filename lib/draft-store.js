...
export function loadDraft() {
    ...
    const legacyIntegratedLesson = data.instructionModel?.id === 'integrated';
    const basics = data.basics ? {
        ...data.basics,
        lessonType: data.basics.lessonType ?? (legacyIntegratedLesson ? 'integrated' : 'single'),
        integrationSubject: data.basics.integrationSubject ?? (legacyIntegratedLesson ? data.instructionModel.integrationSubject ?? '' : ''),
        metadata: normalizeLessonMetadata(data.basics.metadata),
    } : data.basics;
