export const emptyToolEvidence = Object.freeze({
    query: '',
    source: 'unverified',
    verified: false,
    summary: '',
...
export function normalizeToolEvidence(value = {}) {
    return {
        query: String(value?.query ?? ''),
        source: String(value?.source ?? emptyToolEvidence.source),
...
export function buildLessonPlanGenerationRequest(draft = {}) {
    ...
    const isIntegrated = basics.lessonType === 'integrated' || draft.instructionModel?.id === 'integrated';
    const secondarySubject = isIntegrated ? basics.integrationSubject || draft.instructionModel?.integrationSubject || '' : '';
...
export function createGenerationSnapshot(draft = {}) {
    ...
    return {
        basics: {
            ...
            lessonType: basics.lessonType ?? 'single',
            ...
            teachingTools: basics.teachingTools ?? '',
            toolEvidence: normalizeToolEvidence(basics.toolEvidence),
