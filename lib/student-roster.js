import { reviseSubmission } from './grading-generation.js';
...
export function replaceProjectRoster(project, students) {
    ...
    const previousById = new Map(project.students.map(student => [student.id, student]));
    const nextById = new Map(students.map(student => [student.id, student]));
    ...
    const submissions = project.submissions.map(submission => {
        ...
        if (!studentIds.has(submission.studentId)) {
            ...
            return reviseSubmission(submission, { studentId: null, needsStudentLink: true, status: submission.grading ? 'graded' : 'extracted', originalReviewedAt: null, reviewedOriginalRevision: null, approvalRevoked: Boolean(submission.approved || submission.grading) });
        }
        const previous = previousById.get(submission.studentId);
        const next = nextById.get(submission.studentId);
        if (previous && next && ['grade', 'className', 'number', 'name'].every(key => previous[key] === next[key])) return submission;
        return reviseSubmission(submission, { studentName: next?.name ?? submission.studentName, status: submission.grading ? 'graded' : submission.status, originalReviewedAt: null, reviewedOriginalRevision: null, approvalRevoked: Boolean(submission.approved || submission.grading) });
    });
