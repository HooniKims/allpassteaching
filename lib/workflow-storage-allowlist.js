...
const standard = object({ code: text, text, subject: text });
const metadata = object({ date: text, period: text, place: text, className: text, teacherName: text });
const instructionModel = object({ id: text, name: text, reason: text, stages: textArray, integrationSubject: text, integrationStandards: array(standard) });
const toolEvidence = object({ query: text, source: text, verified: boolean, summary: text });
const lessonBasics = object({
    schoolLevel: text, grade: text, subject: text, subjectMode: text, displaySubject: text, mappedSubjects: textArray, lessonType: text, integrationSubject: text,
    mode: text, sessions: number, sessionMinutes: number, intent: text, studentNeeds: text, teachingTools: text, toolEvidence, metadata, error: text,
});
...
const lessonStage = object({
    ...
    minutes: number, materialsAndNotes: textArray, remarks: textArray,
});
...
const unitSequenceItem = object({ session: text, topic: text, learningGoal: text, focus: text });
const detailedPlan = object({
    teacherIntent: text, unitOverview: text, unitGoals: textArray, learnerAnalysis: text, teachingStrategy: text,
    unitSequence: array(unitSequenceItem), boardPlan: textArray, references: textArray,
});
const lessonPlan = object({
    ...
    sessions: array(lessonSession), assessment: array(lessonAssessment), supportStrategies: textArray, reflectionPrompt: text, detailedPlan,
});
...
const worksheetDocument = object({ title: text, instructions: text, studentFields: textArray, sections: array(worksheetSection) });
const worksheetTeacherKey = object({ answers: array(worksheetAnswer) });
export const sanitizeWorksheet = object({
    ...
    document: worksheetDocument, teacherKey: worksheetTeacherKey, sourceHash: text,
});
...
const assessmentTask = object({
    title: text, standards: array(standard), goal: text, role: text, audience: text, situation: text, product: text, successCriteria: text,
...
export const sanitizeAssessment = object({
    ...
    studentSheet: object({ document: worksheetDocument, teacherKey: worksheetTeacherKey }),
    ...
    sourceHash: text, designHash: text, approved: boolean, requiresAssessmentRegeneration: boolean,
});

export function sanitizeAssessmentDesign(value) {
    const safe = sanitizeAssessment(value);
    if (!safe) return undefined;
    delete safe.studentSheet;
    delete safe.cover;
    delete safe.designHash;
    delete safe.approved;
    delete safe.requiresAssessmentRegeneration;
    return safe;
}
...
const record = object({
    ...
    candidateSourceHash: text, candidateTargetBytes: number, candidateInvalidCode: text, candidateInvalidMessage: text, error: text, approved: boolean,
});
...
export function sanitizeRecord(value) {
    ...
    if (!safe.candidateText) {
        ...
        delete safe.candidateTargetBytes;
