const binaryString = /^(?:data:[^,]*;base64,|JVBERi0|UEsDB|iVBORw0KGgo|\/9j\/|R0lGOD)/i;
const encodedBase64 = /^[A-Za-z0-9+/]{16,}={0,2}$/;

function text(value) {
    if (typeof value !== 'string') return undefined;
    const candidate = value.trim();
    const base64 = encodedBase64.test(candidate) && candidate.replace(/=+$/, '').length % 4 !== 1;
    if (/^blob:/i.test(candidate) || binaryString.test(candidate) || base64) return undefined;
    return value;
}
...
const worksheetQuestion = object({ id: text, prompt: text, responseLines: number });
...
export const sanitizeAssessment = object({
    ...
    sourceHash: text, approved: boolean,
});

const gradingCriterion = object({ criterionId: text, score: number, evidence: text, feedback: text });
const grading = object({ criteria: array(gradingCriterion), totalScore: number, summary: text, nextSteps: text });
const submission = object({
    id: text, studentName: text, studentId: nullable(text), needsStudentLink: boolean, fileName: text,
    status: text, extractedText: text, ocrModel: text, pageCount: number, grading: nullable(grading),
    approved: boolean, approvalRevoked: boolean, sourceHash: text, error: text,
});
