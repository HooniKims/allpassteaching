...
const PROCESS_WIDTHS = [760, 1250, 2730, 2550, 650, 2038];
...
function overviewValueParagraphs(row) {
    switch (row.key) {
        case 'standards':
            return row.value.map(({ code, text }) => textParagraph(`[${code}] ${text}`));
        case 'learningGoals':
...
function sessionChildren(documentTitle, session, isLastSession) {
    return [
        ...
        sectionHeading(isLastSession ? '수업 후 연계' : '다음 차시 연결'),
...
export async function buildDocx(plan) {
    ...
    model.sessions.forEach((session, index) => {
        ...
        children.push(...sessionChildren(model.title, session, index === model.sessions.length - 1));
    });
