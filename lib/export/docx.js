...
import { CONTENT_WIDTH, INK, PAPERLOGY, createFormalDocument } from './docx-format.js';
...
function titleParagraph(text) {
    return textParagraph(text, { alignment: AlignmentType.CENTER, bold: true, color: INK, size: 32, after: 100 });
}

function sectionHeading(text) {
    return textParagraph(text, { bold: true, color: INK, size: 22, before: 100, after: 70 });
}
...
function labelCell(label, width) {
    return tableCell([textParagraph(label, { bold: true, color: INK, alignment: AlignmentType.CENTER })], { width, fill: LABEL_FILL });
}
...
function headerRow(columns, widths) {
    return new TableRow({
        ...
        children: columns.map((column, index) => tableCell([
            textParagraph(column.label, { bold: true, color: INK, alignment: AlignmentType.CENTER }),
        ], { width: widths[index], fill: HEADER_FILL })),
    });
}

function labeledBlocks(blocks) {
    return blocks.filter(block => block.items.length).flatMap(block => [
        textParagraph(block.label, { bold: true, color: INK }),
...
function feedbackParagraphs(row) {
    return [
        textParagraph('공통 피드백', { bold: true, color: INK }),
        textParagraph(row.feedback),
        ...row.levelFeedback.flatMap(item => [
            textParagraph(LEVEL_LABELS[item.key], { bold: true, color: INK }),
