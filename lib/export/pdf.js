...
function renderSecondPage(context, session) {
...
function renderDetailedPlan(context, title, detail) {
    addPage(context);
    drawFlowText(context, `${title} · 세안`, {
        font: context.fonts.bold,
        size: 18,
        lineHeight: 23,
        color: COLORS.green,
    ...
    const sections = [
        ['수업자 의도 및 지도 중점', detail.teacherIntent],
        ['단원 개관', detail.unitOverview],
        ['단원 목표', detail.unitGoals.map((item, index) => `${index + 1}. ${item}`).join('\n')],
        ['학습자 분석 및 지도 대책', detail.learnerAnalysis],
        ['수업 모형·설계 틀 적용 전략', detail.teachingStrategy],
        ['단원 지도 계획', detail.unitSequence.map(item => `${item.session} | ${item.topic}\n학습 목표: ${item.learningGoal}\n지도·평가 중점: ${item.focus}`).join('\n\n')],
        ['판서·화면 및 자료 활용 계획', detail.boardPlan.map(item => `• ${item}`).join('\n')],
        ['참고 자료', detail.references.length ? detail.references.map(item => `• ${item}`).join('\n') : '없음'],
    ];
    for (const [sectionTitle, text] of sections) {
        context.state.y -= 8;
        renderSectionTable(context, sectionTitle, singleColumnDefinition(`detail-${sectionTitle}`, text, context.fonts));
    }
}
...
export async function buildPdf(plan, options = {}) {
    ...
    pdf.setTitle(model.documentTitle);
    ...
    if (model.detail) renderDetailedPlan(context, model.documentTitle, model.detail);
    model.sessions.forEach(session => {
        addPage(context);
        renderFirstPage(context, model.documentTitle, session);
        addPage(context);
        renderSecondPage(context, session);
    });
    return pdf.save();
}
