...
function drawAssessmentCover(context, assessment) {
    ...
    heading(context, '평가 목표', 2);
    text(context, `과목 · ${assessment.subject}`, { font: context.fonts.bold, size: 9, lineHeight: 12, after: 2 });
    text(context, assessment.backwardDesign.teacherIntent.desiredResult, { size: 9, lineHeight: 12, after: 3 });
    text(context, `전이 목표 · ${assessment.backwardDesign.transferGoal}`, { size: 9, lineHeight: 12, after: 5 });
    heading(context, '성취기준', 2);
    assessment.task.standards.forEach(standard => text(context, `[${standard.code}] ${standard.text}`, { size: 8.5, lineHeight: 11, after: 2 }));
    heading(context, '수행과제 맥락', 2);
    for (const [label, value] of [['상황', assessment.task.situation], ['역할', assessment.task.role], ['대상', assessment.task.audience], ['산출물', assessment.task.product]]) text(context, `${label} · ${value}`, { size: 8.5, lineHeight: 11, after: 2 });
    heading(context, '제출 안내', 2);
    for (const [label, values] of [['제출 조건', assessment.task.conditions], ['준비물', assessment.task.materials], ['유의점', assessment.task.cautions]]) text(context, `${label} · ${values.join(', ') || '없음'}`, { size: 8.5, lineHeight: 11, after: 2 });
    heading(context, '수행 과정', 2);
    assessment.backwardDesign.checkpoints.toSorted((left, right) => left.order - right.order).forEach(item => text(context, `${item.order}. ${item.title} · ${item.evidence}`, { size: 8.5, lineHeight: 11, after: 2 }));
    heading(context, `평가 기준 · 총 ${assessment.totalPoints}점`, 2);
    assessment.rubric.criteria.forEach(criterion => {
        const scores = assessment.rubric.levels.map((level, index) => `${level.label} · ${criterion.levels[index].score}점`).join(' / ');
        text(context, `${criterion.name} · ${criterion.maxPoints}점 | ${scores}`, { size: 8, lineHeight: 10.5, after: 2 });
    });
    const checklist = assessment.cover.sections.find(section => section.visible && section.type === 'self-checklist');
    if (checklist?.content) {
        heading(context, checklist.label, 2);
        checklist.content.split('\n').filter(Boolean).forEach(item => text(context, `□ ${item}`, { size: 8.5, lineHeight: 11, after: 2 }));
    }
    assessment.cover.sections.filter(section => section.visible && ['custom', 'identity', 'purpose'].includes(section.type) && section.content).toSorted((left, right) => left.order - right.order).forEach(section => {
        heading(context, section.label, 2);
        text(context, section.content, { size: 8.5, lineHeight: 11, after: 3 });
    });
