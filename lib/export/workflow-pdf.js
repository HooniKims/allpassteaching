...
function drawRubric(context, assessment) {
    ...
    assessment.rubric.criteria.forEach((criterion, index) => {
        const continuationHeading = `${index + 1}. ${criterion.name} · 계속`;
        ensure(context, 58);
        ...
        text(context, criterion.description, { size: 9.5, lineHeight: 14, after: 7, continuationHeading });
        assessment.rubric.levels.forEach((level, levelIndex) => labelValue(context, `${level.label} · ${criterion.levels[levelIndex].score}점`, criterion.levels[levelIndex].description, continuationHeading));
        labelValue(context, '관찰 증거', criterion.evidence, continuationHeading);
        context.y -= 10;
    });
}
