...
function drawAssessment(context, assessment) {
    ...
    addPage(context);
    heading(context, `4수준 분석적 루브릭 · 총 ${assessment.totalPoints}점`, 1);
    assessment.rubric.criteria.forEach((criterion, index) => {
        ...
        assessment.rubric.levels.forEach(level => labelValue(context, level.label, criterion.levels[level.id]));
