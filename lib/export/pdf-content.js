...
const PROCESS_WIDTHS = [36, 60, 120, 120, 36, 72, CONTENT_WIDTH - 444];
...
function labelCell(text, fonts) {
    return { text, font: fonts.bold, color: COLORS.ink, fill: COLORS.labelFill, align: 'center' };
}

function overviewText(row) {
    switch (row.key) {
        case 'standards':
            return row.value.map(({ code, text, subject }) => `${subject ? `[${subject}] ` : ''}[${code}] ${text}`).join('\n');
        case 'learningGoals':
...
export function processDefinition(process, fonts) {
    return {
        ...
        headerStyle: { font: fonts.bold, fontSize: 8, lineHeight: 9.8, padding: 2.5, color: COLORS.ink, minimumHeight: 20 },
...
export function assessmentDefinition(assessment, fonts) {
    return {
        ...
        headerStyle: { font: fonts.bold, fontSize: 8, lineHeight: 9.8, padding: 3, color: COLORS.ink, minimumHeight: 20 },
