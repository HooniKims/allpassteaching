...
import { CONTENT_WIDTH as DOCX_CONTENT_WIDTH, createFormalDocument, GREEN, INK, PAPERLOGY } from './docx-format.js';
import { buildHwpxHeader, CONTENT_WIDTH as HWPX_CONTENT_WIDTH, HWPX_STYLE, paragraphXml, sanitizeXmlText, tableXml } from './hwpx-style.js';
...
function hwpRubricTable(ids, assessment) {
    const widths = [12000, HWPX_CONTENT_WIDTH - 12000];
    const rows = [
        [hwpHeaderCell('평가영역', widths[0]), hwpHeaderCell('수준별 기준', widths[1])],
        ...assessment.rubric.criteria.map(criterion => [
            hwpCell([
                hwpLabel(criterionSummary(criterion)),
                hwpBody(criterion.description),
                hwpBody(`성취기준 · ${criterion.standardCodes.join(', ')}`),
                hwpBody(`확인 증거 · ${criterion.evidence}`),
            ], widths[0]),
            hwpCell(assessment.rubric.levels.flatMap(level => {
                const current = criterionLevel(criterion, level);
                return [hwpLabel(`${level.label} · ${current.score}점`), hwpBody(current.description)];
            }), widths[1]),
        ]),
    ];
    return tableXml(ids, { columnWidths: widths, rows });
...
function documentHeaderCell(text, width) {
    return documentCell([documentParagraph(text, { bold: true, color: GREEN, alignment: AlignmentType.CENTER })], width, { fill: 'DDE9E1', verticalAlign: VerticalAlign.CENTER });
}
...
function documentStandardTable(assessment) {
    ...
    for (const standard of assessment.task.standards) {
        rows.push(new TableRow({ cantSplit: true, children: [
            documentCell([documentParagraph(`[${standard.code}]`, { bold: true, color: GREEN })], widths[0]),
...
function documentRubricTable(assessment) {
    ...
    for (const criterion of assessment.rubric.criteria) {
        rows.push(new TableRow({
            ...
            children: [
                documentCell([
                    documentParagraph(criterionSummary(criterion), { bold: true, color: GREEN }),
                    documentParagraph(criterion.description),
                    documentParagraph(`성취기준 · ${criterion.standardCodes.join(', ')}`, { size: 16, color: GREEN }),
                    documentParagraph(`확인 증거 · ${criterion.evidence}`, { size: 16 }),
                ], widths[0]),
                ...assessment.rubric.levels.map((level, index) => {
                    const current = criterionLevel(criterion, level);
                    return documentCell([
                        documentParagraph(`${level.label} · ${current.score}점`, { bold: true, color: GREEN }),
...
export async function buildAssessmentRubricDocx(assessment) {
    const children = [
        documentParagraph(rubricTitle(assessment), { alignment: AlignmentType.CENTER, bold: true, color: GREEN, size: 32, after: 80 }),
        documentParagraph(`${assessment.subject} · 총 ${assessment.totalPoints}점 · ${assessment.rubric.levels.length}수준`, { alignment: AlignmentType.CENTER, bold: true, color: GREEN, size: 20, after: 180 }),
        documentParagraph('성취기준', { bold: true, color: GREEN, size: 22, before: 60, after: 70 }),
        documentStandardTable(assessment),
        documentParagraph('점수형 분석적 루브릭', { bold: true, color: GREEN, size: 22, before: 180, after: 70 }),
...
export async function buildAssessmentRubricXlsx(assessment) {
    ...
    sheet.getCell('A1').font = { name: PAPERLOGY, size: 16, bold: true, color: { argb: 'FF176F5B' } };
    ...
    for (let row = 6; row <= sheet.rowCount; row += 1) {
        ...
        for (let column = 1; column <= lastColumn; column += 1) {
            ...
            cell.font = { name: PAPERLOGY, size: 10, color: { argb: 'FF26342D' } };
