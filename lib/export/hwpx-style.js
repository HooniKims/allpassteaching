...
const MIN_ROW_HEIGHT = 2000;
const COMPACT_CHAR_HEIGHT = 900;
const COMPACT_LINE_HEIGHT = 1125;
...
function estimatedCellHeight(cell) {
    ...
    return Math.max(MIN_ROW_HEIGHT, lineCount * COMPACT_LINE_HEIGHT + CELL_VERTICAL_MARGIN);
}

export function tableXml(ids, definition) {
    const rowHeights = definition.rows.map(row => Math.max(...row.map(estimatedCellHeight)));
    const rowXml = definition.rows.map((row, rowIndex) => {
        ...
        const cells = row.map(cell => {
            ...
            const result = `<hp:tc name="" header="${cell.header ? 1 : 0}" hasMargin="1" protect="0" editable="0" dirty="1" borderFillIDRef="${cell.borderFillIDRef ?? HWPX_STYLE.border.body}">
              ...
              <hp:cellSz width="${cell.width}" height="${rowHeights[rowIndex]}"/><hp:cellMargin left="120" right="120" top="80" bottom="80"/>
            </hp:tc>`;
            colAddr += colSpan;
            return result;
        }).join('');
