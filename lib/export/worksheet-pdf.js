export function drawWorksheet(context, worksheet, includeTeacherKey, tools) {
    ...
    const drawResponseBox = (question, number) => {
        if (ensure(context, question.responseAreaHeight + 8)) continuation(number);
    ...
    const responseHeight = question => {
        ...
        if (question.type === 'table-chart' || question.type === 'drawing-diagram') return question.responseAreaHeight + 8;
