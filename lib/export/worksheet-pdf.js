export function drawWorksheet(context, worksheet, includeTeacherKey, tools) {
    ...
    const continuation = (number, label = '응답') => text(context, `${number}번 문항 ${label} (계속)`, { font: context.fonts.bold, size: 10, color: colors.ink, lineHeight: 14, after: 6 });
    ...
    const drawQuestion = (question, number) => {
        ...
        context.onDraw?.({ kind: 'worksheet-question-start', pageIndex: context.pageIndex, questionId: question.id, questionNumber: number, y: context.y });
