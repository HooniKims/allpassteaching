...
const LONG_OVERVIEW_KEYS = new Set(['unitTitle', 'standards', 'learningGoals', 'essentialQuestion', 'materials']);
...
function sessionXml(ids, documentTitle, session, isLastSession) {
    const parts = [];
    ...
    parts.push(
        ...
        sectionHeading(ids, isLastSession ? '수업 후 연계' : '다음 차시 연결'),
...
function buildSectionXml(baseSection, model) {
    ...
    const content = model.sessions.map((session, index) => sessionXml(ids, model.title, session, index === model.sessions.length - 1)).join('\n');
...
export async function buildHwpx(plan) {
    ...
    zip.file('Contents/header.xml', buildHwpxHeader(baseHeader));
    zip.file('Contents/section0.xml', buildSectionXml(baseSection, model));
