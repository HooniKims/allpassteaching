...
export function worksheetMessages(lessonPlan, selectedFormatId, generationRequest) {
    ...
    // 형식이 정한 모든 섹션에 문항이 하나 이상 가도록 예시 문항을 채웁니다. 예시가 한 섹션뿐이면 모델도 한 섹션만 만듭니다.
    while (questions.length < format.sections.length) {
        questions.push(createWorksheetQuestion(generationRequest.questionTypes[questions.length % generationRequest.questionTypes.length], questions.length, [lessonPlan.standards[0].code]));
    }
    const shapeSections = format.sections.map((title, index) => ({
        id: `section-${index + 1}`,
        title,
        purpose: '이 섹션의 학습 목적',
        questions: questions.filter((_question, questionIndex) => questionIndex % format.sections.length === index),
    }));
    ...
    const shape = {
        ...
        generationRequest,
        document: { title: '학습지 제목', instructions: '학생 안내', studentFields: ['이름', '학년·반', '날짜'], sections: shapeSections },
        teacherKey: { answers: questions.map(question => ({ questionId: question.id, answer: '교사용 예시 답안과 확인할 핵심 근거' })) },
    };
    return [
        { role: 'system', content: `당신은 한국 교사의 학습지 설계 전문가입니다. ${modelDirective} ${integrationDirective} 지도안의 materials나 활동에 디지털 도구가 있으면 학생이 그 도구로 무엇을 하고 무엇을 남기는지 문항과 응답 공간에 반영하고, 지도안에 없는 도구는 새로 지어내지 마세요. 교사의 지도안을 그대로 요약하지 말고 학생이 사고·기록·표현할 문항과 충분한 응답 공간을 설계하세요. 선택 형식은 ${format.name}(${format.id})이며 바꾸지 마세요. 형식 원리: ${format.guide} 필수 섹션 흐름: ${format.sections.join(' → ')}. document.sections는 이 흐름의 모든 섹션을 같은 순서와 같은 제목으로 정확히 ${format.sections.length}개 만들고 각 섹션에 문항을 하나 이상 두세요. 문항 prompt에 '1.'이나 '문항 1'처럼 번호를 붙이지 마세요. 번호는 학습지에서 자동으로 붙습니다. standards와 generationRequest는 입력값을 글자 하나도 바꾸지 말고 그대로 반환하세요. 요청 유형 ${generationRequest.questionTypes.join(', ')}을 각각 한 문항 이상 포함하세요. 모든 문항은 standards에 있는 성취기준 코드를 하나 이상 연결하세요. multiple-choice-5는 choices를 정확히 다섯 개 두고, table-chart와 drawing-diagram은 responseAreaHeight를 80~400으로 두며, 나머지 유형은 responseLines를 1~16으로 두세요. 모든 문항 id는 고유해야 하고 teacherKey에는 각 문항과 같은 questionId의 예시 답안을 정확히 하나씩 넣으세요. 정답이 하나가 아닌 문항은 판단 기준과 가능한 응답 예를 쓰세요. 교사 추가 요구: ${generationRequest.additionalRequirements || '없음'}. 다음 JSON 키와 구조만 사용하세요: ${JSON.stringify(shape)}` },
...
export function gradingMessages({ assessment, studentName, extractedText, elements = [], elementsTruncated = false, visualAnalysisStatus = 'not_requested' }) {
    ...
    return [
        { role: 'system', content: `당신은 교사의 수행평가 채점 보조자입니다. 제공된 학생 제출물 텍스트, OCR 요소, 현재 루브릭만 사용하세요. criteria는 제공된 모든 criterion id를 정확히 한 번씩 같은 순서로 반환하세요. 확실한 텍스트 근거가 있으면 status=scored로 현재 criterion levels 중 하나의 levelId와 그 수준에 정의된 정확한 score를 선택하세요. 각 수준 설명과 학생 근거를 대조해 가장 잘 맞는 수준 하나를 고르고, 그 수준의 score를 그대로 사용하세요(임의로 점수를 바꾸지 마세요). evidence는 제출물에 실제로 연속해서 존재하는 문구이며 reason은 그 근거가 선택 수준 설명에 부합하는 이유, feedback은 다음 성장을 위한 구체적인 제안입니다. 모든 평가영역에는 판단 확신도를 0과 1 사이 숫자 confidence로 반드시 포함하세요(scored·teacher_review 모두). 근거가 분명하면 0.85 이상, 애매하면 0.85 미만으로 두세요. 수식·도표·그림, 낮은 신뢰도, 생략된 요소, Enhanced 실패, 연결되지 않은 근거 또는 불확실한 판독은 status=teacher_review로 두고 selectedLevelId와 score는 반드시 null로 하며 reviewReason을 쓰세요. 불확실성을 낮은 점수로 바꾸지 마세요. sourceRefs는 제공된 element id와 page만 사용하세요. 같은 OCR 요소를 두 평가영역의 근거로 연결하지 마세요. 평가영역마다 그 영역의 수행이 가장 잘 드러나는 서로 다른 요소를 고르세요. teacherConfirmed는 항상 false입니다. 제출물에 없는 수행·태도·의도·인성을 추정하지 마세요. 총점은 반환하지 마세요. 서버가 검증·계산합니다. 다음 JSON 구조만 사용하세요: ${JSON.stringify(shape)}` },
