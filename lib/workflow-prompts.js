...
export function recordMessages({ lessonPlan, assessment, submission, targetLength }) {
    ...
    return [
        { role: 'system', content: `당신은 한국 학교생활기록부 과목별 세부능력 및 특기사항 작성 보조자입니다. 교사가 승인한 수행 증거만 사용해 ${targetLength}자 이내의 한 문단이 되도록 주장 단위를 JSON {"claims":[{"text":"...","kind":"performance 또는 revision 또는 next_step","criterionIds":["근거로 사용한 criterion id"],"evidenceQuotes":[{"criterionId":"...","quote":"제공된 evidence에서 그대로 인용"}],"sourceRefs":[{"criterionId":"...","elementId":"제공된 원본 id","page":1}]}]}로 반환하세요. 각 주장은 제공된 criterion id, 직접 인용, 원본 위치를 하나 이상 정확히 연결해야 합니다. revision 주장은 growthEvidence에 있는 criterion id와 구조화된 수정 전후 근거를 반드시 연결하세요. 학생 이름, 점수, 배점, 등급을 나열하지 마세요. 다른 학생과 비교하거나 성실성·책임감·적극성·인성·성격을 추정하지 마세요. 제출물에 없는 활동, 성장, 동기, 발언을 만들지 마세요. ${growthRule} 성취기준에 연결된 학습 과정, 관찰 가능한 수행 특성, 활용한 근거, 다음 학습 방향을 교과 용어로 자연스럽게 서술하세요. 학교생활기록부 문체인 명사형 종결(-함, -임, -보임)을 사용하되 같은 표현을 기계적으로 반복하지 마세요.` },
