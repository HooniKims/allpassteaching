function guide(stages, teacherMoves, studentEvidence, antiPatterns, sources, options = {}) {
    return Object.freeze({
        stages, teacherMoves, studentEvidence, antiPatterns, sources,
        applicationMode: options.applicationMode ?? 'lesson-sequence',
        designCheckMode: options.designCheckMode ?? 'all',
    });
}
...
export const instructionModelGuides = Object.freeze({
    ...
    integrated: guide(
        ['공통 맥락·문제', '교과 관점 탐구', '관점 통합', '적용·성찰'],
        ['두 개 이상 교과가 함께 필요한 실제 맥락과 공통 문제를 제시한다.', '교과별 핵심 개념·자료·방법으로 문제를 탐구하게 한다.', '교과 관점의 관계와 차이를 연결해 새로운 설명이나 해결안을 구성하게 한다.', '통합한 이해를 새 맥락에 적용하고 각 교과의 기여와 한계를 성찰하게 한다.'],
        ['공통 문제와 필요한 교과 관점을 연결한 문제 지도를 만든다.', '교과별 개념·자료·방법으로 탐구한 증거를 남긴다.', '두 관점을 연결한 설명이나 해결 산출물을 만든다.', '새 맥락에 적용하고 교과별 기여와 한계를 설명한다.'],
        ['교과 이름만 여러 개 붙이고 실제 개념·방법의 연결이 없는 나열식 활동'],
        [{ title: 'IB MYP Interdisciplinary Learning', url: 'https://www.ibo.org/programmes/middle-years-programme/curriculum/interdisciplinary/' }],
    ),
    tpack: guide(
        ['내용·목표 확인', '교수법 선택', '기술 적합성 검토', '통합·맥락 점검'],
        ['성취기준과 핵심 내용, 학생이 보여줄 학습 증거를 먼저 확인한다.', '학생의 사전 지식과 오개념에 맞는 교수·학습 방법을 선택한다.', '기술의 교육적 이점과 접근성·개인정보·대체 수단을 함께 검토한다.', '내용·교수법·기술이 학급 맥락에서 서로 맞물리는지 점검하고 조정한다.'],
        ['학습 목표와 핵심 내용을 자신의 말로 확인한다.', '선택한 학습 활동으로 내용을 탐구하고 설명한다.', '기술을 사용한 이유와 다른 방법과의 차이를 기록한다.', '학습 결과와 기술 활용이 목표에 도움이 되었는지 성찰한다.'],
        ['도구를 먼저 정한 뒤 학습 목표와 교수법을 끼워 맞추는 설계'],
        [{ title: 'TPACK Official Site', url: 'https://tpack.org/' }, { title: 'Learning Activity Types', url: 'https://tpack.org/lats/index.html' }],
        { applicationMode: 'design-check' },
    ),
    samr: guide(
        ['대체(Substitution)', '증강(Augmentation)', '수정(Modification)', '재정의(Redefinition)'],
        ['같은 과제를 기능 변화 없이 디지털 도구로 대신할 필요가 있는지 점검한다.', '기능 개선이 피드백·접근·표현을 실제로 돕는지 점검한다.', '협업·피드백·표현 방식이 실질적으로 달라지도록 과제를 재설계한다.', '기술이 없으면 어려웠던 새로운 수행이 목표에 필요한지 접근성·안전과 함께 점검한다.'],
        ['같은 학습 목표를 다른 도구로 수행한 결과를 비교한다.', '추가 기능이 학습 과정과 결과에 준 변화를 설명한다.', '달라진 협업·피드백·표현 과정과 산출물을 근거로 제시한다.', '새롭게 가능해진 수행과 실제 학습 효과를 근거로 성찰한다.'],
        ['학습 목표와 학생의 접근성을 무시하고 가장 높은 재정의 수준만 추구하는 설계'],
        [{ title: 'Ruben Puentedura: SAMR and Redefinition', url: 'https://www.hippasus.com/rrpweblog/archives/2013/08/15/SAMR_RedefinitionInContext.pdf' }, { title: 'NSW Education: SAMR Framework', url: 'https://education.nsw.gov.au/content/dam/main-education/teaching-and-learning/professional-learning/scan/media/documents/vol-42/Scan_42-3_Term3_2023.pdf' }],
        { applicationMode: 'design-check', designCheckMode: 'select-one' },
    ),
});

export function instructionModelGuide(modelId) {
    const value = instructionModelGuides[modelId];
    if (!value) throw new Error(`지원하지 않는 수업 설계입니다: ${modelId}`);
    return value;
}
