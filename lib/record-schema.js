...
export function unsupportedRecordClaimMessages(value) {
    ...
    const messages = [];
    if (/(?:반|학급)에서\d+등|(?:반|학급)에서가장높은점수|친구들중가장우수|다른친구들에비해|다른학생들에비해|또래(?:보다|에비해)|(?:학급|반)(?:상위|하위)|상위권|하위권/.test(compact)) messages.push('학급 내 순위·최상급 또는 다른 학생과의 비교를 기록하지 마세요.');
    if (/\d+(?:퍼센트|%|점)|(?:백|일백)점만점에[영공일이삼사오육칠팔구십백]+점/.test(compact)) messages.push('점수·백분율·만점 환산 결과를 기록하지 마세요.');
    if (/(?:매우)?부지런|모범적(?:인)?태도|성실(?:한|함|하게)?|근면(?:한|함)?|책임감|친절(?:한|함)?|배려심?|리더십|주도성|끈기|협동심|협력적태도|자발적|소극적|차분한태도|바른태도|성격/.test(compact)) messages.push('승인된 수행 증거로 확인할 수 없는 성격·인성·태도를 추정하지 마세요.');
