...
export function createAssessmentFallback(lessonPlan, assessmentRequest) {
    ...
    const disciplinaryPoints = Math.floor(outcomePoints / 4);
    const integratedCriteria = integrationGroups.length === 2 && disciplinaryPoints >= assessmentRequest.levelCount - 1
        ? [
            criterionShape('criterion-primary', `${integrationGroups[0].subject} 교과 근거`, 'outcome', disciplinaryPoints, `${integrationGroups[0].subject} 교과의 개념·자료·방법을 사용한 수행 증거`),
            criterionShape('criterion-secondary', `${integrationGroups[1].subject} 교과 근거`, 'outcome', disciplinaryPoints, `${integrationGroups[1].subject} 교과의 개념·자료·방법을 사용한 수행 증거`),
            criterionShape('criterion-integration', '교과 관점 통합', 'outcome', outcomePoints - disciplinaryPoints * 2, '두 교과의 근거를 연결해 만든 설명, 해결안 또는 공동 산출물'),
        ].map((criterion, index) => ({ ...criterion, standardCodes: index < 2 ? integrationGroups[index].codes : standards.map(item => item.code) }))
        : null;
    const outcomeCriteria = integratedCriteria ?? (assessmentRequest.includeProcessInScore
        ? [criterionShape('criterion-outcome', '결과 증거', 'outcome', outcomePoints, '산출물에서 직접 찾을 수 있는 결과 증거')]
        : [
            criterionShape('criterion-outcome-1', '결과 증거 1', 'outcome', Math.ceil(outcomePoints / 2), '산출물에서 직접 찾을 수 있는 결과 증거'),
            criterionShape('criterion-outcome-2', '결과 증거 2', 'outcome', Math.floor(outcomePoints / 2), '산출물에서 직접 찾을 수 있는 결과 증거'),
        ]);
    const rubricCriteria = assessmentRequest.includeProcessInScore
        ? [...outcomeCriteria, criterionShape('criterion-process', '피드백 반영 과정', 'process', processTargetPoints, '초안, 피드백 표시, 수정본과 수정 이유')]
        : outcomeCriteria;
