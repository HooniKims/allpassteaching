...
export function createAssessmentFallback(lessonPlan, assessmentRequest) {
    ...
    const criterionShape = ({ id, name, kind, maxPoints, evidence }) => {
        ...
        return {
            ...
            levels: levels.map((level, index) => ({
                levelId: level.id,
                score: Math.max(0, maxPoints - index),
    ...
    const outcomeCriteria = integratedCriteria ?? (assessmentRequest.includeProcessInScore
        ...
        : [
            criterionShape({ id: 'criterion-outcome-1', name: '결과 증거 1', kind: 'outcome', maxPoints: Math.ceil(outcomePoints / 2), evidence: '산출물에서 직접 찾을 수 있는 결과 증거' }),
            criterionShape({ id: 'criterion-outcome-2', name: '결과 증거 2', kind: 'outcome', maxPoints: Math.floor(outcomePoints / 2), evidence: '산출물에서 직접 찾을 수 있는 결과 증거' }),
        ]);
