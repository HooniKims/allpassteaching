...
export function assessmentDesignMessages(lessonPlan, assessmentRequest) {
    ...
    return [
        { role: 'system', content: `당신은 한국 학교의 수행평가 설계 전문가입니다. ${approach.name} 원리(${approach.promptDirective})에 따라 교사가 수정할 수행과제와 분석적 루브릭 초안을 만드세요. 학생용 문제지와 표지는 아직 만들지 마세요. 성취기준 코드와 원문, teacherIntent, 교사가 정한 총점·수준 수·과정 배점·생성 설정을 정확히 보존하세요. 평가영역 배점 합은 ${assessmentRequest.totalPoints}점이어야 하고 각 수준은 산출물이나 수행 과정에서 직접 관찰할 수 있는 서로 다른 기술로 쓰세요. 모든 성취기준과 평가영역을 evidenceMap에 양방향으로 연결하고 피드백 뒤 수정 체크포인트를 두세요. JSON 이외의 문장은 쓰지 마세요. 다음 키와 구조만 사용하세요: ${JSON.stringify(shape)}` },
