...
`StandardsStep`은 모든 과목에서 추천 목록과 직접 목록을 동시에 렌더링한다. 직접 목록은 현재 학교급·학년군·선택 과목의 공식 성취기준 전체를 대상으로 코드·내용 검색을 제공하고 결과를 자르지 않는다. 사회·역사는 여기에 `전체/지리/일반사회` 또는 역사 단일 영역 범위를 적용하고, 융합 수업은 선택 교과별 필터를 제공한다. 두 목록은 같은 선택 배열을 사용하며 10개를 넘겨 추가하려 하면 `role="alert"`로 안내한다. 추천 실패 시 직접 목록과 선택 상태를 유지한다.
