export function GradingEditor({ assessment, submission, onChange, onSourceSelect = () => {} }) {
    ...
    const applyGrading = grading => {
        ...
        onChange({ ...submission, grading: { ...grading, totalScore }, originalReviewedAt: null, approved: false, approvalRevoked: submission.approved || submission.approvalRevoked || false });
    };
        ...
        <div className="grading-criteria">{submission.grading.criteria.map((criterion, index) => {
            ...
            return <fieldset key={criterion.criterionId}><legend>{rubric?.name ?? criterion.criterionId}</legend>
                ...
                <div className="grading-source-links">{criterion.sourceRefs?.length
                    ? criterion.sourceRefs.map((sourceRef, sourceIndex) => <span className="grading-source-link" key={`${sourceRef.elementId}-${sourceIndex}`}><button type="button" className="text-button" aria-label={`${criterion.evidence} 원본에서 보기`} onClick={() => onSourceSelect(sourceRef)}>원본에서 보기</button><button type="button" className="text-button" aria-label={`${criterion.evidence} 원본 연결 삭제`} onClick={() => updateCriterion(index, { sourceRefs: criterion.sourceRefs.filter((_, refIndex) => refIndex !== sourceIndex) })}>연결 삭제</button></span>)
                    : <span>원본 위치 연결 안 됨</span>}</div>
            </fieldset>;
        })}</div>
