...
export function OcrGradingStage({ assessment, students = EMPTY_COLLECTION, submissions, onStudentsChange = () => {}, onDeleteStudent = () => {}, records = EMPTY_COLLECTION, onChange }) {
    ...
    useEffect(() => {
        const detached = submissions.filter(item => item.originalAttached === true && !files.has(item.id));
    ...
    return <section className="workflow-stage workflow-stage--wide">
        ...
        {submissions.some(item => (files.has(item.id) || item.file) && ['pending', 'ocr_error'].includes(item.status)) && <div className="ocr-start-actions"><button type="button" disabled={busy || fileBusy} onClick={() => processOcr(submissions.filter(item => (files.has(item.id) || item.file) && ['pending', 'ocr_error'].includes(item.status)))}>연결한 답안 PDF OCR 시작</button></div>}
        ...
        <div className="submission-list">{submissions.map((submission, index) => {
            ...
            <div className="submission-item__head"><label>학생 이름 {index + 1}<input value={studentName} disabled={Boolean(linkedStudent)} onChange={event => updateOne(submission.id, { studentName: event.target.value, approved: false })}/></label><div><strong>{submission.fileName}</strong><span>{submission.originalAttached === false ? '원본 PDF 다시 연결 필요' : submission.status === 'pending' ? 'OCR 대기' : submission.status === 'extracting' ? 'OCR 처리 중…' : submission.status === 'extracted' ? `${submission.pageCount}쪽 OCR 완료` : submission.status === 'grading' ? '루브릭 채점 중…' : stale ? '다시 채점 필요' : submission.approved ? '교사 승인 완료' : submission.grading ? '채점 검토 필요' : '처리 확인 필요'}</span></div><button type="button" className="text-button" onClick={() => { files.remove(submission.id); onChange(current => current.filter(item => item.id !== submission.id)); }}>{studentName} 삭제</button></div>
            {submission.originalAttached === false && <p className="stale-notice" role="status"><strong>원본 PDF 다시 연결</strong><span>새로고침으로 원본 파일이 사라졌습니다. <span className="nowrap">OCR·채점 초안은</span> 남아 있지만 원본을 다시 연결하기 전에는 승인할 수 없습니다.</span></p>}
            {submission.error && <p className="item-error" role="alert">{submission.error}</p>}
            {submission.status === 'ocr_error' && (files.has(submission.id) || submission.file) && <button type="button" className="secondary-button" onClick={() => processOcr([submission])}>{studentName} OCR 다시 시도</button>}
            ...
            {submission.grading && <><GradingEditor assessment={assessment} submission={{ ...submission, studentName }} onChange={next => updateOne(submission.id, next)}/>{!validGrading && <p className="form-alert" role="alert">모든 점수는 평가 요소별 배점 범위 안에 있어야 하며 근거와 피드백을 입력해야 합니다.</p>}<div className="approval-actions"><p>AI 채점은 초안입니다. OCR 원문과 모든 근거를 확인한 뒤 승인해주세요.</p><button type="button" disabled={stale || !validGrading || submission.originalAttached === false} className={submission.approved ? 'secondary-button' : ''} onClick={() => updateOne(submission.id, { approved: !submission.approved, status: submission.approved ? 'graded' : 'approved', approvalRevoked: false })}>{submission.approved ? '승인 취소' : `${studentName} 채점 승인`}</button></div></>}
        </article>;})}</div>
