...
function newSubmission(file, index) { return { id: `submission-${Date.now()}-${index}`, studentName: studentNameFromFile(file), fileName: file.name, file, status: 'pending', extractedText: '', ocrModel: '', pageCount: 0, grading: null, approved: false, error: '' }; }

export function OcrGradingStage({ assessment, submissions, onChange }) {
    ...
    const grade = async submission => {
        ...
        try {
            const response = await fetch('/api/grade-submission', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assessment, studentName: submission.studentName, extractedText: submission.extractedText }) });
            const body = await response.json().catch(() => ({}));
    ...
    return <section className="workflow-stage workflow-stage--wide">
        <header className="workflow-stage__header"><div><p className="eyebrow">4단계 · OCR·채점</p><h1>학생 PDF를 읽고 루브릭으로 검토해요</h1><p>여러 PDF를 선택하면 최대 두 개씩 처리합니다. OCR 원문과 채점 근거를 교사가 직접 확인하고 승인합니다.</p></div></header>
        <aside className="ocr-privacy-note"><strong>처리 전 확인</strong><span>PDF는 Upstage에 전송되며 원본은 저장하지 않습니다. 학생 이름·OCR·채점 결과는 현재 탭에만 임시 보관되어 <span className="nowrap">새로고침 후 복구되고</span>, <span className="nowrap">탭을 닫으면 사라집니다.</span></span></aside>
        ...
        <div className="submission-list">{submissions.map((submission, index) => {
            ...
            <div className="submission-item__head"><label>학생 이름 {index + 1}<input value={submission.studentName} onChange={event => updateOne(submission.id, { studentName: event.target.value, approved: false })}/></label><div><strong>{submission.fileName}</strong><span>{submission.status === 'pending' ? 'OCR 대기' : submission.status === 'extracting' ? 'OCR 처리 중…' : submission.status === 'extracted' ? `${submission.pageCount}쪽 OCR 완료` : submission.status === 'grading' ? '루브릭 채점 중…' : stale ? '다시 채점 필요' : submission.approved ? '교사 승인 완료' : submission.grading ? '채점 검토 필요' : '처리 확인 필요'}</span></div><button type="button" className="text-button" onClick={() => onChange(current => current.filter(item => item.id !== submission.id))}>{submission.studentName} 삭제</button></div>
            {submission.error && <p className="item-error" role="alert">{submission.error}</p>}
            {submission.status === 'ocr_error' && submission.file && <button type="button" className="secondary-button" onClick={() => processOcr([submission])}>{submission.studentName} OCR 다시 시도</button>}
