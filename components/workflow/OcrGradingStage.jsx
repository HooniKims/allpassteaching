...
export function OcrGradingStage({ assessment, students = EMPTY_COLLECTION, submissions, onStudentsChange = () => {}, onDeleteStudent = () => {}, records = EMPTY_COLLECTION, onChange }) {
    ...
    useEffect(() => {
        ...
        onChange(current => current.map(item => detachedIds.has(item.id) ? { ...item, originalAttached: false, originalReviewedAt: null, reviewedOriginalRevision: null, approved: false, approvalRevoked: Boolean(item.approved || item.grading) } : item));
    ...
    useEffect(() => {
        ...
        onChange(current => current.map(item => ids.has(item.id) ? { ...item, originalReviewedAt: null, reviewedOriginalRevision: null, approved: false, approvalRevoked: true } : item));
    ...
    const processOcr = async targets => {
        ...
        await runWithConcurrency(targets, 2, async item => {
            ...
            try {
                ...
                apply(item.id, { ...body, status: 'extracted', error: '', file: undefined, grading: null, originalReviewedAt: null, reviewedOriginalRevision: null, confirmedElementIds: [], approved: false });
    ...
    const grade = async submission => {
        ...
        try {
            const response = await fetch('/api/grade-submission', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                assessment, studentName, extractedText: submission.extractedText, elements: submission.elements ?? [],
            ...
            updateOne(submission.id, { status: 'graded', grading: body.grading, originalReviewedAt: null, reviewedOriginalRevision: null, confirmedElementIds: [], approved: false, approvalRevoked: false, sourceHash: body.grading.sourceHash, error: '' });
    ...
    const finalize = async submission => {
        const response = await fetch('/api/grade-submission', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
            mode: 'finalize', assessment, extractedText: submission.extractedText, elements: submission.elements ?? [],
    ...
    return <section className="workflow-stage workflow-stage--wide">
        ...
        <div className="submission-list">{submissions.map((submission, index) => {
            ...
            const validGrading = gradingContentIsValid(assessment, submission.grading, submission.extractedText, submission.elements);
            const linkedStudent = students.find(student => student.id === submission.studentId);
            const studentName = linkedStudent?.name || submission.studentName;
            ...
            {submission.grading && <SubmissionReviewWorkspace assessment={assessment} submission={{ ...submission, studentName }} studentName={studentName} fileUrl={files.get(submission.id)?.packetUrl ?? ''} stale={stale} validGrading={validGrading} onFinalize={finalize} onPatch={patch => updateOne(submission.id, patch)}/>}
        </article>;})}</div>
