...
export function StudentRosterEditor({ students, submissions = EMPTY_COLLECTION, records = EMPTY_COLLECTION, onChange, onDeleteStudent }) {
    ...
    const editIssues = useMemo(() => validateRoster(students), [students]);
    const confirmedStudent = students.find(student => student.id === confirmId);
    useEffect(() => {
    ...
    const addStudent = () => {
        ...
        const previous = students.at(-1);
        onChange([...students, createStudent({ grade: previous?.grade ?? '', className: previous?.className ?? '', number: students.length + 1, name: '' })]);
    };
    const updateStudent = (id, field, value) => {
        onChange(students.map(student => student.id === id ? { ...student, [field]: field === 'number' && value !== '' ? Number(value) : value } : student));
    };
    const requestDelete = student => {
        ...
        else onDeleteStudent(student.id);
    };
    ...
    return <section className="student-roster" aria-labelledby="student-roster-heading">
        <div className="student-roster__heading">
            ...
            <strong aria-label={`등록 학생 ${students.length}명, 최대 ${MAX_STUDENTS}명`}>현재 {students.length}명 / 최대 {MAX_STUDENTS}명</strong>
        </div>
        <div className="student-roster__tools">
            ...
            <button type="button" disabled={busy || students.length >= MAX_STUDENTS} onClick={addStudent}>학생 직접 추가</button>
        </div>
        ...
        {editIssues.length > 0 && students.length > 0 && <div className="roster-issues roster-issues--editing" role="status"><strong>직접 입력 중 확인할 내용</strong><ul>{editIssues.map(item => <li key={`${item.code}-${item.row}-${item.column}`}>{item.row}번 학생 · {item.column}: {item.message}</li>)}</ul></div>}
        <div className="student-roster__list" aria-label="학생 명단 편집">
            {students.map((student, index) => <fieldset className="student-roster__row" key={student.id}>
                ...
                <div className="student-roster__row-actions">
                    <button type="button" className="secondary-button" aria-label={`${student.name || `${index + 1}번 학생`} 위로 이동`} disabled={index === 0} onClick={() => onChange(move(students, index, -1))}>위로</button>
                    <button type="button" className="secondary-button" aria-label={`${student.name || `${index + 1}번 학생`} 아래로 이동`} disabled={index === students.length - 1} onClick={() => onChange(move(students, index, 1))}>아래로</button>
                    <button type="button" className="text-button" aria-label={`${student.name || `${index + 1}번 학생`} 삭제`} onClick={() => requestDelete(student)}>삭제</button>
                </div>
            </fieldset>)}
        </div>
        {!students.length && <p className="empty-state">학생을 직접 추가하거나 Excel 입력 양식으로 한 번에 불러오세요.</p>}
