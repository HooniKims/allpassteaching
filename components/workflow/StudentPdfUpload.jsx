...
export function StudentPdfUpload({ students, submissions, onChange, onBusyChange = () => {} }) {
    ...
    return <section className="student-pdf-upload" aria-labelledby="student-pdf-upload-title" aria-busy={busy}>
        ...
        {mode === MODES.individual && <div className="pdf-upload-panel">
            <label className="inline-check pdf-cover-check"><input type="checkbox" checked={hasCoverPerStudent} onChange={event => setHasCoverPerStudent(event.target.checked)}/><span className="pdf-cover-check__copy">각 개별 PDF의 첫 페이지가 <span className="nowrap">이 학생의 수행평가 안내 표지</span></span></label>
