...
export function RecordsStage({ lessonPlan, assessment, students = [], submissions, records, onChange }) {
            ...
            await runWithConcurrency(targets, 2, async submission => {
                ...
                try {
                    ...
                    if (!authorizationResponse.ok || !recordContext?.payload || !/^[a-f0-9]{64}$/.test(recordContext?.token ?? '')) {
                        throw new Error(authorizationBody.message || '현재 명단과 승인 결과를 확인하지 못했습니다.');
                    }
                    if (controller.signal.aborted) return;
                    ...
                    if (typeof body?.record?.text !== 'string' || !body.record.text.trim() || body.record.text.length > targetLength) throw new Error('생성 결과 형식을 확인하지 못했습니다. 다시 시도해주세요.');
                    merge(identity, current => hadRecord
                        ? { status: 'done', regenerationStatus: 'done', error: '', previousText: current?.text ?? '', candidateText: body.record.text, candidateClaims: body.record.claims ?? [], candidateEvidenceCriterionIds: body.record.evidenceCriterionIds ?? [], candidateSourceHash: nextSourceHash, candidateTargetLength: targetLength }
                    ...
                    merge(identity, current => hadRecord
                        ? { status: 'done', regenerationStatus: cancelled ? '' : 'error', error: cancelled ? '' : error instanceof Error ? error.message : '세특 초안을 만들지 못했습니다.', approved: current?.approved ?? false }
    ...
    return <section className="workflow-stage workflow-stage--wide" aria-busy={busy}>
        ...
        <div className="record-list">{approved.map(submission => {
            ...
            const candidateCurrent = record?.candidateSourceHash === currentSourceHash && record?.candidateTargetLength === targetLength && (record?.candidateText?.length ?? 0) <= targetLength;
            return <article className="record-item" key={submission.id}>
                ...
                {record && <><label className="record-text-field">{student.name} 세특 초안<textarea rows="7" value={record.text ?? ''} disabled={record.status === 'generating'} maxLength={targetLength} onChange={event => updateRecord(submission.id, { text: event.target.value, claims: [], evidenceCriterionIds: [], approved: false })}/></label>
                    <RecordDraftComparison studentName={student.name} record={record} candidateCurrent={candidateCurrent} onApply={() => { updateRecord(submission.id, { previousText: '', text: record.candidateText, claims: record.candidateClaims ?? [], evidenceCriterionIds: record.candidateEvidenceCriterionIds ?? [], sourceHash: record.candidateSourceHash, candidateText: '', candidateClaims: [], candidateEvidenceCriterionIds: [], candidateSourceHash: '', candidateTargetLength: 0, regenerationStatus: '', error: '', approved: false }); setBatchMessage(`${student.name} 새 초안을 적용했습니다.`); }} onKeep={() => { updateRecord(submission.id, { previousText: '', candidateText: '', candidateClaims: [], candidateEvidenceCriterionIds: [], candidateSourceHash: '', candidateTargetLength: 0, regenerationStatus: '', status: 'done', error: '' }); setBatchMessage(`${student.name} 기존 문장을 유지했습니다.`); }}/>
