...
export function AssessmentCoverEditor({ value, onChange }) {
    ...
    const renderSection = section => {
        ...
        if (section.type === 'subject') return <><p><strong>과목</strong> · {value.subject}</p>{introduction}</>;
        if (section.type === 'transfer-goal') return <><p>{value.backwardDesign.transferGoal}</p>{introduction}</>;
        ...
        if (section.type === 'grasps') return <>{introduction}<dl className="cover-live-list"><div><dt>상황</dt><dd>{value.task.situation}</dd></div><div><dt>역할</dt><dd>{value.task.role}</dd></div><div><dt>대상</dt><dd>{value.task.audience}</dd></div><div><dt>산출물</dt><dd>{value.task.product}</dd></div></dl></>;
