...
import { instructionModelTypeLabel } from '@/data/instruction-models';
...
function EditableCell({ label, headerId, descriptionId, value, onChange, onBlur, multiline = false, readOnly = false }) {
    ...
    return <td data-label={label} headers={headerId}>
        {multiline && readOnly
            ? <div className="readonly-multiline" role="textbox" aria-label={label} aria-describedby={descriptionId} aria-readonly="true" tabIndex="0">{value}</div>
            : <Control aria-label={label} aria-describedby={descriptionId} value={value} onChange={onChange} onBlur={onBlur} readOnly={readOnly} />}
    </td>;
}

function EditableTimingCell({ prefix, headerId, metadata, updateMetadata }) {
    return <td headers={headerId}>
        <div className="overview-timing">
            <input aria-label={`${prefix} 수업 일자`} type="date" value={metadata.date ?? ''} onChange={event => updateMetadata('date', event.target.value)} />
            <input aria-label={`${prefix} 교시`} type="number" min="1" max="12" placeholder="교시" value={metadata.period ?? ''} onChange={event => updateMetadata('period', event.target.value)} />
        </div>
    </td>;
}
...
export function OverviewTable({ plan, session, overview, descriptionId, onPlanChange, onSessionChange }) {
    ...
    const instructionModelLabel = instructionModelTypeLabel(plan.instructionModel);

    return <table className="formal-table overview-table" aria-label={`${prefix} 수업 개요`}>
        <caption>수업 개요</caption>
        <tbody>
            <tr className="overview-table__centered">
                <th id={headerId('date')} scope="row">일시</th>
                <EditableTimingCell prefix={prefix} headerId={headerId('date')} metadata={metadata} updateMetadata={updateMetadata} />
            ...
            <tr className="overview-table__centered">
                ...
                <EditableCell label={`${prefix} 과목`} headerId={headerId('subject')} value={rowValue(overview, 'subject')} readOnly />
            ...
            <tr className="overview-table__centered">
                <th id={headerId('instructionModel')} scope="row">{instructionModelLabel}</th>
                <EditableCell label={`${prefix} ${instructionModelLabel}`} headerId={headerId('instructionModel')} value={rowValue(overview, 'instructionModel')} readOnly />
            ...
            <tr>
                ...
                <EditableCell label={`${prefix} 성취기준`} headerId={headerId('standards')} value={standards.map(item => `${item.subject ? `[${item.subject}] ` : ''}[${item.code}] ${item.text}`).join('\n')} multiline readOnly />
