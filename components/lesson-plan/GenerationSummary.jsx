...
export function GenerationSummary({ draft, changed, loading, onEdit, onRegenerate }) {
    ...
    const instructionModelLabel = instructionModelTypeLabel(draft.instructionModel);
    return <section className="generation-summary" aria-labelledby="generation-summary-title">
        ...
        <dl>
            ...
            <div><dt>{draft.instructionModel?.id === 'integrated' ? `${draft.basics.displaySubject || draft.basics.subject} 성취기준` : '성취기준'}</dt><dd>{draft.standards.map(item => item.code).join(', ')}</dd></div>
            {draft.instructionModel?.id === 'integrated' && <div><dt>{draft.instructionModel.integrationSubject} 성취기준</dt><dd>{draft.instructionModel.integrationStandards?.map(item => item.code).join(', ')}</dd></div>}
            <div><dt>{instructionModelLabel}</dt><dd>{draft.instructionModel?.name}</dd></div>
        </dl>
